...
| O mesmo "failed to lock vertex buffer" com o dgVoodoo e o feed funcionando (Black Mesa, 26/09/2026: `bms.exe` LAA, sem DXVK, `DGD3D11ROOT` no `ReShade.log`; o erro vem ~1 s depois de o `dlss5-feed.log` entregar os primeiros quadros) | Memória do processo 32-bit: o `ReShade.log` do jogo tinha **44** `Successfully compiled` — a pasta `reshade-shaders` inteira (SweetFX, MartysMods, REST…) compilada e criada dentro do `bms.exe`, junto com o jogo, o dgVoodoo, o feed e o driver, quando o preset só usa dois | Em jogo 32-bit com o Feeder o `ReShade.ini` sai com `[GENERAL] SkipLoadingDisabledEffects=1` (sobem só o provedor de MV e o `DLSS5_Feed` do preset) e `[OVERLAY] AutoSavePreset=0` (o F6 não regrava o preset; sem isso, fechar o jogo com o DLSS 5 desligado faria o `DLSS5_Feed.fx` nem carregar na volta). Item 5d da verificação conta os efeitos no `ReShade.log` (`CargaDeEfeitos`). A chave `EffectLoadSkipping` que o programa gravava antes não existe no ReShade 6 |
| Tela de carregamento parada com o banner "Compilando (40 efeitos restantes)… O aplicativo pode ficar sem resposta" (Black Mesa, 26/09/2026) | O ReShade do jogo compilando a pasta inteira num processo 32-bit (o próprio ReShade limita a 4 threads em 32-bit por falta de endereço) | Idem: `SkipLoadingDisabledEffects=1`. Sem reinstalar: painel do ReShade → Configurações → Geral → "Carregar apenas efeitos ativados" ("Load only enabled effects") |
