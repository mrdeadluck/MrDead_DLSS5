...
| Jogo congela ao ligar o DLSS 5; `dlss5-feed.log` termina em `host spawned` sem `host connected`; host log tem `game pid connected` e nenhum `build:` | Tela cheia exclusiva: o jogo trava no aperto de mão quando o host sobe (Enslaved, 10/09/2026) | Jogo em janela/sem borda; rota C: opção "dgVoodoo em janela sem borda" + Instalar de novo; painel via "Show as texture" |
