...
- Relato de 23/09/2026: o plano recusava ("já existe um D3D8.dll que não é o dgVoodoo"); o usuário renomeou o `d3d8.dll` do fix e instalou. O DLSS 5 rodou, mas sem o carregador o `Silent_Hill_3_PC_Fix.dll` nunca subia — o jogo abria em resolução mínima e o menu de opções não abria. Devolvendo o `d3d8.dll` do fix, o jogo voltava ao normal sem DLSS 5.
- Arranjo: o carregador **fica** como `D3D8.dll` e o dgVoodoo (o `MS\x86\D3D8.dll` do kit) entra como **`d3d8R.dll`** (`GameProfile.D3d8ViaD3d8R`, decidido pelo texto UTF-16 `d3d8R.dll` dentro do `D3D8.dll`; ver `CarregadorD3d8R`). O `dgVoodoo.conf` sai no perfil padrão (todas as resoluções), não no "Legado": o fix foi feito rodando no d3d8 do Windows, que enumera todas, e a resolução personalizada dele costuma ser a do monitor. O resto é a rota C normal: dgVoodoo → D3D11 → `dxgi.dll` (ReShade x86) → Feeder addon32 + `host64\`.
...
- Nome ocupado: `D3D9.dll` do DxWrapper → encadeia por `RealDllPath` (6.x, Dead Space 2); `D3D8.dll` de uma mod com d3d8to9 (SH2 Enhanced Edition) → o dgVoodoo entra como `D3D9.dll` atrás dela; `D3D8.dll` que é o carregador do Silent Hill 3 PC Fix → o dgVoodoo entra como `d3d8R.dll`, o nome que o carregador procura; qualquer outro wrapper com o nome → o plano recusa, porque sobrescrever é apostar com o jogo. O dgVoodoo funciona renomeado: acha o `dgVoodoo.conf` pela pasta.
...
| Silent Hill 3: com o DLSS 5 o jogo abre em resolução mínima e o menu de opções não abre; devolvendo o `d3d8.dll` do fix, volta ao normal sem DLSS 5 | O `d3d8.dll` do Silent Hill 3 PC Fix foi trocado (ou renomeado) pelo `D3D8.dll` do dgVoodoo: é ele que carrega o `Silent_Hill_3_PC_Fix.dll` | Devolver o `d3d8.dll` do fix e instalar de novo: o dgVoodoo entra como `d3d8R.dll` atrás do carregador. À mão: renomear o `D3D8.dll` do dgVoodoo para `d3d8R.dll` e pôr o do fix de volta |
