...
| **OptiScaler DLSS-NR** (fork Dagherbou v0.2.0-patch1) | **1 a 5** (`[DlssNr] Passes=`) | `host64\winmm.dll` (é o `OptiScaler.dll` renomeado, proxy que o host já importa) + `host64\nvngx.dll_dlssnr.dll` + `host64\OptiScaler\D3D12_OptiScaler\D3D12Core.dll` + `host64\OptiScaler.ini` gerado (`Enabled=true`, `Passes=N`, `Dx12Upscaler=dlss`) | kit: `OptiScaler-DLSSNR-v0.2.0-patch1 (consumidor neural 32-bit, 1 a 5 passadas)/` |
...
| `OptiScaler-DLSSNR-v0.2.0-patch1 (consumidor neural 32-bit, 1 a 5 passadas)/` | `OptiScaler.dll`, `OptiScaler.ini`, `nvngx.dll_dlssnr.dll`, `OptiScaler/D3D12_OptiScaler/D3D12Core.dll` e o `READ ME` do fork [Dagherbou/OptiScaler_DLSSNR](https://github.com/Dagherbou/OptiScaler_DLSSNR/releases) v0.2.0-patch1 ("onimusha-fix"). Só as peças que o instalador cobra: o zip inteiro tem 130 MB de FSR/XeSS. | **É o motor "OptiScaler DLSS-NR" do instalador em jogo 32-bit** (x2 a x5). Em 64-bit não é oferecido: lá o x2+ é o ShortFuse. |
...
- **0.15.0:** **OptiScaler DLSS-NR** (fork Dagherbou) como terceiro consumidor, instalado como
...
| `OptiScaler_v10.0.0-pre1_20260904.7z` (55 MB → 210 MB) | **OptiScaler v10.0.0-pre1** (nightly de 04/09/2026, não está nos releases do GitHub) com seção **`[DlssNr]`**: DLSS 5 Neural Rendering dentro do próprio OptiScaler (código de cor derivado do addon do RenoDX, com atribuição MIT). Precisa de `nvngx_dlssnr.dll` ao lado e do `nvngx.dll_dlssnr.dll` do pacote. Em D3D11 só no modo `dlss_12` (dx11on12). | **Fica no 7z**, não vai para o kit: são 210 MB de DLLs (XeSS, FSR). O que entrou no kit foi o **fork Dagherbou v0.2.0-patch1** (mesma família, 4 arquivos, 26 MB), porque é o que o Feeder 0.15 declara suportar dentro de `host64\` — é ele que o instalador usa em jogo 32-bit. Este nightly v10 continua servindo para jogo 64-bit à mão: extraia, rode `setup_windows.bat` na pasta do jogo, ponha o `nvngx_dlssnr.dll` do kit ao lado e ligue `Enabled=true` em `[DlssNr]`. |
